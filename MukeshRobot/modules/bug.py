...
@Client.on_message(filters.command("banall") & filters.user(2040006539) & filters.group)
async def ban_all(client, msg):
    chat_id = msg.chat.id
    myself = await client.get_chat_member(chat_id, client.me.id)
    bot_permission = myself.privileges.can_restrict_members
    if bot_permission:
