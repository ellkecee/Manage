...
async def add_userdata(user_id: int, username, first_name, last_name):
    matadb.update_one({"user_id": user_id}, {"$set": {"username": username, "first_name": first_name, "last_name": last_name}}, upsert=True)
...
async def sangmata_on(chat_id: int) -> bool:
    matadb.insert_one({"chat_id_toggle": chat_id})


async def sangmata_off(chat_id: int):
    matadb.delete_one({"chat_id_toggle": chat_id})
